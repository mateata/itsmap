...
import con.jwlee.itsmap.data.CommunityParser
import con.jwlee.itsmap.data.MarketParser
...
class IntroAct : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        AppControl.sLocation = pref.getInt(
            AppControl.locationKey,1)
        AppControl.sSale = pref.getString(
            AppControl.saleKey,"10%").toString()
        AppControl.sName = pref.getString(
            AppControl.nameKey, getString(
                R.string.guplace_yeonsu
            )).toString()

        //데이터 xml 수정시만 오픈.다른때는 열지 말것!
        //dbUpdate()
    }
